...
def plott(folder,collection,collection2):
       ...
       "xtick.major.size" : 5,"xtick.major.width" : 2,"xtick.minor.size" : 3,"xtick.minor.width" : 1}
        ...
        axx.text(0.5,1.18,'Warm sector',weight="bold",size=25,horizontalalignment='center',verticalalignment='center',\
        ...
        frontloc=axx.scatter(xfront,0,marker='*',color='b',s=400,clip_on=False) #just for legend
        axx.scatter(xfront,0,marker='*',color=col,s=400,clip_on=False)


        c3=axx.contour(latt0,H0.H.data*hscale,
                        (DTHDTCN.DTHDTCN*3600*24).data,\
                levels=[4,5,8],colors='k',alpha=1,extend='max',linewidths=3)
        ...
        c3=axx.contourf(latt0,H0.H.data*hscale,
                ...
                levels=np.arange(0,1901,100),cmap='YlOrRd',alpha=0.9,extend='max') #.where((CNVMFC.CNVMFC*3600*24)>=300)
        ...
        arrow=axx.scatter([], [], marker='$\longleftrightarrow$', c="b", s=1500)
        legend=axx.legend([leg3[0],pblh,leg4[0],arrow,frontloc],\
                ['Conv. diabatic heating','PBLH','Moisture convergence','SST front size','SST front location'],\
        ...
        axx1.text(tx,ty,'b',weight="bold",size=25,horizontalalignment='center',verticalalignment='center',\
        ...
        (PRECCON.PRECCON*24*3600).plot(ax=axx2_,c='b',linewidth=linewidth,label='Convective precipitation')
        ...
        arr=axx3.annotate('',size=15,xy=(lat1, ypos), xytext=(lat2, ypos),ha='center', va='center',
            arrowprops=dict(arrowstyle='<->',color=col, lw=4),annotation_clip=False)
        axx3.text((lat2+lat1)/2,ypos+0.01,str(dx)+' km',size=20,color=col,horizontalalignment='center',verticalalignment='bottom')
        ...
        axx.text(0.5,1.18,'Cold sector',weight="bold",size=25,horizontalalignment='center',verticalalignment='center',\
                transform = axx.transAxes,\
                bbox=dict(boxstyle="round", fc="white", ec="black", pad=0.2))
        ...
        frontloc=axx.scatter(xfront,0,marker='*',color='k',s=400,clip_on=False) #just for legend
        axx.scatter(xfront,0,marker='*',color=col,s=400,clip_on=False)
        
        c3=axx.contour(latt0,H0.H.data*hscale,
                        (DTHDTCN.DTHDTCN*3600*24).data,\
                levels=[2,3,4],colors='k',alpha=1,extend='max',linewidths=3)
        axx.clabel(c3, inline=1, fontsize=20)
        leg3,_=c3.legend_elements()#c3.set_label('Dabatic heating')
        ...
        c4=axx.contour(latt0,H0.H.data*hscale,
                        (-divV.V*QV.QV*1e3*3600*24).data,\
                levels=[2.5,3,3.5,4],colors='r',alpha=1,extend='max',linewidths=3)
        axx.clabel(c4, inline=1, fontsize=20)
        leg4,_=c4.legend_elements()

        c3=axx.contourf(latt0,H0.H.data*hscale,
                        (CNVMFC.CNVMFC*3600*24).data,\
                levels=np.arange(0,1201,100),cmap='YlOrRd',alpha=0.9,extend='max') #.where((CNVMFC.CNVMFC*3600*24)>=300)
        ...
        axx1.text(tx,ty,'e',weight="bold",size=25,horizontalalignment='center',verticalalignment='center',\
                transform = axx1.transAxes,\
                bbox=dict(boxstyle="round", fc="white", ec="black", pad=0.2))
...
plt.savefig('/path/to/your/plots/ExtDataFig5.png')
