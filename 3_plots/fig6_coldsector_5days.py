...
def plott(endname):
        ...
        name='CNVMFC_2000m'
        file=path+name+endname
        CNVMFC_2000m=xr.open_dataset(file).sel(lon=slice(lonmin,lonmax)).sel(lat=slice(latmin,latmax))

        name='DTHDTCN_2000m'
        file=path+name+endname
        DTHDTCN_2000m=xr.open_dataset(file).sel(lon=slice(lonmin,lonmax)).sel(lat=slice(latmin,latmax))
       ...
       "xtick.major.size" : 5,"xtick.major.width" : 2,"xtick.minor.size" : 3,"xtick.minor.width" : 1}
        ...
        c2=(CNVMFC_2000m.CNVMFC*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,1101,100),cmap='YlOrRd',extend='max',add_colorbar=False)#,norm=norm)
        ...
        axx50.xaxis.set_ticks([150,152,154,156]) 
        ...
        c2=(DTHDTCN_2000m.DTHDTCN*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,6.1,0.5),cmap='YlOrRd',extend='max',add_colorbar=False)
        ...
        c2=(PRECCON.PRECCON*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,1.01,0.1),cmap='Blues',extend='max',add_colorbar=False)     
...
plott('_mean_16dec21dec_SW0_2D.nc') # This refers to 5-days mean for southward winds
plt.savefig('/path/to/your/plots/fig6.png')
plt.close()
