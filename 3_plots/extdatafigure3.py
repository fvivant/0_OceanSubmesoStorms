...
def plott(endname, endname2, figsize, scaling):
        ...
        CNVMFC_3000m = xr.open_dataset(file).sel(lon=slice(lonmin, lonmax)).sel(
            lat=slice(latmin, latmax))

        name = 'DTHDTCN_3000m'
        file = path + name + endname
        DTHDTCN_3000m = xr.open_dataset(file).sel(
        ...
        cbar = plt.colorbar(mappable=c2,
                            ...
                            aspect=35)
        cbar.set_label(
            "Convective Mass Flux at $3$ km [kg.m$^{-2}$.day$^{-1}$]",
            labelpad=labelpad)
        cbar.ax.tick_params(labelsize=tick_font_size)

        lonnn, lattt = np.meshgrid(U10M.lon.data, U10M.lat.data)

        q = axx50.quiver(lonnn[::alphax, ::alphay],
                         lattt[::alphax, ::alphay],
        ...
        arr = axx50.annotate('',
                             ...
                             arrowprops=dict(arrowstyle='<->', color='k', lw=6),
                             annotation_clip=False)
        axx50.text(lon1, (lat2 + lat1) / 2 - 0.3,
                   str(dx) + ' km',
                   size=smallsize,
                   zorder=8,
                   horizontalalignment='center',
                   ...
                   bbox=dict(boxstyle="round", fc="white", ec="black", pad=0.2))
        axx50.text((lon2 + lon1) / 2, (lat1 + 0.08),
                   str(dx) + ' km',
                   size=smallsize,
                   zorder=8,
                   horizontalalignment='center',
        ...
        axx50.text(lon1, (lat2 + lat1) / 2,
                   ...
                   verticalalignment='bottom',
                   bbox=dict(boxstyle="round", fc="white", ec="black", pad=0.2))
        axx50.text((lon2 + lon1) / 2, (lat1 + 0.19),
                   str(dx) + ' km',
                   size=smallsize,
                   zorder=8,
                   horizontalalignment='center',
...
plott('_rms_16dec21dec_NW0sup_2D.nc', '_rms_decjan_NW0sup_2D.nc', figsize,
      scaling)

dpi_target = 500  # desired dpi
dpi = dpi_target / scaling  # to get a target dpi after resizing
figname = 'ExtDataFigure3.png'
