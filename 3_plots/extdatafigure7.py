...
def plott(title, folder, lonsel, collection, collection2, figsize, scaling):
        zmax = 4000
        rollat = 6
        rolz = 1
        ...
        name = 'wrho'
        wrho = xr.open_dataset(folder + name + collection).rolling(
            lat=rollat, depth=rolz,
            center=True).mean().sel(lat=slice(latmin, latmax))

        name = 'DTHDT_DELP'
        DTHDT_DELP = xr.open_dataset(folder + name + collection).rolling(
            lat=rollat, depth=rolz,
            center=True).mean().sel(lat=slice(latmin, latmax))

        name = 'P'
        P = xr.open_dataset(folder + name +
                            collection).sel(lat=slice(latmin, latmax))

        name = 'PRECTOT'
        PRECTOT = xr.open_dataset(folder + name +
                                  collection).sel(lat=slice(latmin, latmax))
        ...
        c3 = axx.contourf(
            latt0,
            H0.H.data * hscale,
            ((wrho.wrho - wrho.wrho.mean(dim='lat').data[:, None]) * 3600 *
             24).data,
            levels=np.arange(-1000, 1001, 50),
            cmap=cmocean.cm.balance,
            alpha=1,
            extend='both',
            linewidths=3)  #cmocean.cm.thermal
        ...
        cbar.set_label(
            "Total vertical mass flux anomaly $(\\rho w)_{anom}$ [kg.m$^{-2}$.day$^{-1}$]",
            labelpad=labelpad)
        ...
        (PRECTOT.PRECTOT * 24 * 3600).plot(ax=axx2_,
                                           c='b',
                                           linewidth=linewidth,
        ...
        c3 = axx.contourf(
            ...
            ((wrho.wrho - wrho.wrho.mean(dim='lat').data[:, None]) * 3600 *
             24).data,
            levels=np.arange(-3200, 3201, 100),
            cmap=cmocean.cm.balance,
            alpha=1,
            extend='both',
            linewidths=3)  #np.arange(-1000,1001,50)
        ...
        legend = axx.legend([leg3[0], pblh, leg4[0], arrow, frontloc], [
            'Tot. diabatic heating', 'PBLH', 'Moisture convergence',
...
plott('title', folder, lonsel, collection, collection2, figsize, scaling)

dpi_target = 500  # desired dpi
dpi = dpi_target / scaling  # to get a target dpi after resizing
figname = 'ExtDataFigure7.png'
