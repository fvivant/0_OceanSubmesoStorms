...
def plott(endname):
       ...
       "xtick.major.size" : 5,"xtick.major.width" : 2,"xtick.minor.size" : 3,"xtick.minor.width" : 1}
        ...
        c2=(CNVMFC_3000m.CNVMFC*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,1001,25),cmap='YlOrRd',extend='max',add_colorbar=False)#,norm=norm)
        ...
        c2=(DTHDTCN_3000m.DTHDTCN*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,4.1,0.25),cmap='YlOrRd',extend='max',add_colorbar=False)
        ...
        c2=(PRECCON.PRECCON*3600*24).plot.contourf(ax=axx50,\
                levels=np.arange(0,4.6,0.25),cmap='Blues',extend='max',add_colorbar=False)     
...
plott('_mean_decjan_NW0sup_2D.nc')
plt.savefig('/path/to/your/plots/ExtDataFig4.png')
plt.close()
